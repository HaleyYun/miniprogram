...
Page({
  ...
  onLoad(options) {
    ...
    console.log(timestamp + '第二本相册开始时间')
    wx.setStorageSync('twoStartTime', timestamp)
    this.setData({
      picArr: wx.getStorageSync('setPicRemberNoTwo')
    })
    this.data.playUrl = wx.createInnerAudioContext({
      useWebAudioImplement: true
    });
    this.data.playUrl.src = app.globalData.ossGamesImgUrl + "audio/Test1/tts-BrainCA-LITE-Test1-0008.mp3"
  ...
  //       wx.setStorageSync('setPicRemberNoThree', res.data.show_all)
  //     }
  //   })
  // },
  chooseTureType(e) {
      ...
      if (index === getIndex) {
        if (that.data.questionArr.length > 0) {
          if (getType.split('_')[0] === that.data.questionArr[0].type) { // 判断是否为点击的正确图片
            ...
            console.log(that.data.rightClickImgArr + '11111111')
          } else {
            clearTimeout(that.data.timer)
        ...
        that.data.timer = setTimeout(function () {
          ...
          if (that.data.questionArr.length === 0) {
            ...
            wx.setStorageSync('trueClickCountTwoRP', newArr.length)
            wx.redirectTo({
              url: '/packageHome/pages/gameSku/photoTwoSecond/index',
            })
          } else {

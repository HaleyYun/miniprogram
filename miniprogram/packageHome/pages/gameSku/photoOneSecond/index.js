...
Page({
  ...
  chooseTureType(e) {
    ...
    that.data.picArr.forEach((item, index) => {
      ...
      if (getClickIndex === index && item.choose === 1) { // 点击正确时
        ...
        // that.data.rightClickImgArr.push(that.data.clickCountIndex)
        // that.setData({
        //   rightClickImgArr: that.data.rightClickImgArr
        // })
        isClickTure = true
        that.setData({
        ...
        console.log(that.data.rightClickImgArr)
        ...
        // that.data.errClickImgArr.push(that.data.clickCountIndex)
        isClickTure = false
        that.setData({
          ...
          // errClickImgArr: that.data.errClickImgArr
        })
    ...
    if (that.data.clickCountIndex >= 6 && that.data.isChooseTypeTure) {
      ...
      console.log(JSON.stringify(that.data.rightClickImgArr) + '点击对类型')
      ...
      wx.setStorageSync('rememberPicOneErrNumber', that.data.rightClickImgArr.length)
      wx.redirectTo({
        url: '/packageHome/pages/gameSku/photoOneThird/index',
      })
      return
    }
  ...
  filterNowType() {
    let getPci = wx.getStorageSync('setPicRemberNoOne')
